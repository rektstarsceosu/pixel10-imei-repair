!!!!~DISCLAIMER~!!!!  
This is just a for educational purposes. IMEI modification may be ILLEGAL depending on where you live. DO NOT use IMEIs you don't own.  
...
I finally found a way to run deprecated pixel9 umts_router commands on pixel10 series of devices  

It may work for other pixel devices but i only tested it on pixel 10 pro (blazer), pixel 10 series has the literally same chip with pixel 9 series, but because google decided to remove some functionality with A16 some commands do not work on stock modem.img.  

## requirements
- rooted device with some way to run shell commands (termux or adb shell).
- oldest modem.img from pixel 9 devices. you can get it from here or extract it from googles official OTA archives.
- this modem restart wrapper (or any other way to restart the modem, explained below)  


### 1. get the original devinfo.img and fix it

```
adb shell # or your preffered way to get shell
su
...
you can use any hex editor or `sed` if you preffer, they are ASCII encodes strings:  
```
...
send the devinfo_modified.img into device
```
dd if=/sdcard/devinfo_modified.img of=/dev/block/by-name/devinfo  

```

### 3. reboot the device and and also enter factory mode
...
```
you will see a red 'Factory' warning when device powers on.  
...
you can extract modem with [payload-dumper-go](https://github.com/ssut/payload-dumper-go)  
...
losetup -f
# on pixel10 pro it starts from loop55, NUMBER MAY BE DIFFRENT ADJUST ACCORINGLY!!!
losetup /dev/block/loop55 /sdcard/modem9.img
```
mount the old image and replace the modem.img inside the modem.img (i don't know why there is another modem.img inside modem.img but this modem.img is where commands execute from.)  
```
...
basicly you need to send IOCTL_TRIGGER_CP_CRASH to modem  
```c
...
after modem restarts AT+GOOGGETIMEISHA will be availabe  
```
...
### Sources:
```
- claude.ai for writing me the wrapper for restarting the modem
...
- my hatred against corpos taking away our right to modify/repair our devices and anyone who sells these knowledge with money
